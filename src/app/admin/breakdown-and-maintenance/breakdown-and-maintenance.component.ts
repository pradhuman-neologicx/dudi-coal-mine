...
import { catchError, debounceTime, distinctUntilChanged, switchMap, tap, map, takeUntil } from 'rxjs/operators';
...
export interface MachineShiftOption {
...
export class BreakdownAndMaintenanceComponent implements OnInit, OnDestroy {
  ...
  employees$ = new Subject<any[]>(); // We'll manually emit values to this
  ...
  machinesList: MachineShiftOption[] = [];
  ...
  setupEmployeeSearch() {
    this.employeeInput$.pipe(takeUntil(this.destroy$)).subscribe(term => {
  ...
  fetchBreakdownTypes(): void {
    this.breakdownTypeService.getPublicBreakdownTypes().pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
  ...
  fetchBreakdownData() {
    ...
    this.breakdownTypeService.getBreakdowns(this.pageSize, this.currentPage, '', filters).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        if (res && res.status === 200) {
          // Update Dashboard
          if (res.dashboard) {
            this.dashboardStats = { ...this.dashboardStats, ...res.dashboard };
          }
  ...
  createChart1() {
    const ctx = document.getElementById('breakdownTrendChart') as HTMLCanvasElement;
    if (!ctx) return;
    if (this.chart1) this.chart1.destroy();
  ...
  createChart2() {
    const ctx = document.getElementById('downtimeMachineChart') as HTMLCanvasElement;
    if (!ctx) return;
    if (this.chart2) this.chart2.destroy();
    
    const downtimeData: any[] = (this.dashboardStats as any).downtime_by_machine || [];
  ...
  createChart3() {
    const ctx = document.getElementById('breakdownCategoryChart') as HTMLCanvasElement;
    if (!ctx) return;
    if (this.chart3) this.chart3.destroy();
    
    const categoryData: any[] = (this.dashboardStats as any).category_analysis || [];
    ...
    this.chart3 = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          data: data,
          backgroundColor: ['#3b82f6', '#ef4444', '#eab308', '#22c55e', '#a855f7', '#f97316', '#64748b', '#ec4899']
        }]
      },
      options: { responsive: true, maintainAspectRatio: false }
    });
  ...
  createChart4() {
    const ctx = document.getElementById('reliabilityRankingChart') as HTMLCanvasElement;
    if (!ctx) return;
    if (this.chart4) this.chart4.destroy();
    
    const rankingData: any[] = (this.dashboardStats as any).reliability_ranking || [];
  ...
  viewModal(ticket: any) {
    ...
    this.breakdownTypeService.getBreakdownById(ticketId).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        if (res && res.status === 200) {
  ...
  openModal(ticket?: any) {
    if (ticket) {
      ...
      this.breakdownTypeService.getBreakdownById(this.editingTicketId).pipe(takeUntil(this.destroy$)).subscribe({
        next: (res: any) => {
          if (res && res.status === 200 && res.data) {
            const data = res.data;
            ...
            this.entryDate = formatForInput(data.breakdown_date_time);
            ...
            if (this.selectedEmployee) {
              // Pre-populate the dropdown array so ng-select can match and display it
              // We use reported_by as both id and name assuming the backend stored the name directly
              this.employees$.next([{ id: this.selectedEmployee, name: this.selectedEmployee }]);
            } else {
              this.employees$.next([]);
            }
            ...
            if (this.entryDate) {
              this.shiftPlanningService.shiftPlanFilterByDate(this.entryDate).pipe(takeUntil(this.destroy$)).subscribe((shiftRes: any) => {
  ...
  submitEntry() {
    ...
    if (this.editingTicketId) {
      this.breakdownTypeService.updateBreakdown(this.editingTicketId, payload).pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => {
          this.closeModal();
          this.fetchBreakdownData();
        },
      ...
      this.breakdownTypeService.createBreakdown(payload).pipe(takeUntil(this.destroy$)).subscribe({
        next: (res) => {
          this.closeModal();
          this.fetchBreakdownData();
        },
  ...
  onFileSelected(event: Event) {
    ...
    if (file) {
      ...
      this.importResult = null;
      this.breakdownTypeService.importBreakdowns(file).pipe(takeUntil(this.destroy$)).subscribe({
        next: (res: any) => {
