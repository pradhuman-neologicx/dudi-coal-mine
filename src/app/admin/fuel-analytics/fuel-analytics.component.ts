...
@Component({
  ...
  styleUrls: ['./fuel-analytics.component.scss']
})
export class FuelAnalyticsComponent implements OnInit, OnDestroy {
  fuelData: any = null;
  isLoading = true;
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      ...
      this.shiftFilter = params['shift'] || null;
  ...
  fetchFuelData() {
    ...
    // We reuse the getFleetFuel API to fetch the full fuel report data
    const filters: any = {
    ...
    this.dataSub = this.dashboardService.getFleetFuel(filters).subscribe({
      next: (res: any) => {
        if (res.status && res.data) {
          this.fuelData = res.data;
          // Capture server pagination if available
          if (res.pagination) {
            this.serverPagination = res.pagination;
          } else if (res.data.pagination) {
            this.serverPagination = res.data.pagination;
          } else {
            // Fallback for endpoints that don't return pagination metadata
            // Use basis_count as total items
            const total = res.data.basis_count || res.data.lowest?.length || 0;
      ...
      error: (err: any) => {
        ...
        this.error = 'Failed to load fuel analytics data.';
        this.isLoading = false;
      }
    });
  }
  ...
  changePage(page: number) {
    if (page >= 1 && page <= this.serverPagination.last_page) {
      ...
      this.fetchFuelData(); // Fetch new page from server
    }
  }
