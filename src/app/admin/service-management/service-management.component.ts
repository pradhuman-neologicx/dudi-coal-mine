...
export class ServiceManagementComponent implements OnInit, OnDestroy {
        ...
        next: (res: any) => {
          if (res && res.status === 200 && res.data) {
            ...
            this.activeBreakdowns = list.map((item: any) => ({
              ...
              type:
                item.breakdown_type || item.equipment_category || 'Breakdown',
              description:
                item.description || item.resolution_notes || 'Breakdown logged',
              date: item.breakdown_date_time,
            }));
          }
        },
        error: (err: any) =>
          console.error('Error fetching active breakdowns:', err),
      });
  }
        ...
        (this.searchInput || '').trim(),
        {
          service_type:
            this.filterServiceType === 'All'
              ? null
              : this.filterServiceType === 'General Service'
                ? 'general'
                : 'repair',
        ...
        next: (res: any) => {
          ...
          if (res && res.status === 200 && res.data) {
            const list = Array.isArray(res.data)
              ? res.data
              : res.data?.data || res.data || [];
            this.servicesList = list.map((item: any) =>
              this.parseServiceRecordData(item, item),
            );
            if (res.pagination && res.pagination.total !== undefined) {
              ...
              res.data &&
              res.data.pagination &&
              res.data.pagination.total !== undefined
            ) {
  ...
  initForms() {
    ...
    this.serviceForm
      .get('isBreakdown')
      ?.valueChanges.subscribe((isBreakdown) => {
  ...
  timeComparisonValidator(formGroup: FormGroup) {
    const start = formGroup.get('serviceStartTime')?.value;
    const end = formGroup.get('serviceEndTime')?.value;
    if (start && end) {
      if (end < start) {
  ...
  onBreakdownDateChange(dateTime: string) {
    if (dateTime) {
      this.shiftPlanningService
        .shiftPlanFilterByDate(dateTime)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (res: any) => {
            if (res && res.status === 200 && res.data) {
              ...
              if (data.site) {
                this.breakdownSites = [
                  { id: data.site.id, name: data.site.site_name },
                ];
              } else {
                this.breakdownSites = [];
              }
      ...
      if (control.get('source')?.value === 'Store') {
        ...
        if (currentStoreId) {
          if (firstStoreId === null) {
            ...
            control.get('storeId')?.setErrors({
              ...control.get('storeId')?.errors,
              differentStore: true,
            });
          } else {
            if (control.get('storeId')?.hasError('differentStore')) {
  ...
  onStoreChange(storeId: any, index: number, isInit = false) {
    ...
    if (storeId && !this.storeProductsMap[storeId]) {
      this.storeService
        .getAvailableProductsByStore(storeId)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: (res: any) => {
            if (res && res.status === 200 && res.data) {
  ...
  onInventoryPartSelect(part: any, index: number) {
    ...
    if (part && typeof part === 'string') {
      selectedPart = this.sparePartsList.find(
        (p) => p.name === part || p.product_name === part,
      );
    }
  ...
  confirmDeleteAttachment() {
    if (this.attachmentToDeleteIndex !== null && this.editingServiceId) {
      ...
      if (attachment && attachment.id) {
        ...
        this.serviceRecordService
          .deleteServiceAttachment(this.editingServiceId, attachment.id)
          .pipe(takeUntil(this.destroy$))
          .subscribe({
            next: (res) => {
  ...
  calculateTotalAmount() {
    ...
    if (val.hydraulicOil === 'Yes')
      total += Number(val.hydraulicOilAmount) || 0;
    if (val.gearOil === 'Yes') total += Number(val.gearOilAmount) || 0;
    if (val.fuelFilterChange === 'Yes')
      total += Number(val.fuelFilterChangeAmount) || 0;
    if (val.oilFilterChange === 'Yes')
      total += Number(val.oilFilterChangeAmount) || 0;
  ...
  calculateKPIs(apiSummary?: any) {
    if (apiSummary) {
      this.stats.vehiclesInBreakdown =
        apiSummary.vehicles_in_breakdown?.value || 0;
      this.stats.vehiclesInService =
        apiSummary.vehicles_in_active_service?.value || 0;
    ...
    this.stats.vehiclesInService = this.servicesList.filter(
      (s) =>
        s.status === 'In Progress' ||
        s.status?.toLowerCase() === 'in progress' ||
        s.status?.toLowerCase() === 'in_progress',
    ).length;
    this.stats.totalServicesDone = this.servicesList.filter(
      (s) =>
        (s.serviceType === 'General Service' ||
          s.serviceType?.toLowerCase() === 'general') &&
        (s.status === 'Completed' || s.status?.toLowerCase() === 'completed'),
    ).length;
    this.stats.totalRepairsDone = this.servicesList.filter(
      (s) =>
        (s.serviceType === 'Repair' ||
          s.serviceType?.toLowerCase() === 'repair' ||
          s.serviceType?.toLowerCase() === 'breakdown') &&
        (s.status === 'Completed' || s.status?.toLowerCase() === 'completed'),
    ).length;
  ...
  parseServiceRecordData(data: any, fallback: any = {}): any {
    const isBreakdown =
      data.is_breakdown_service !== undefined
        ? data.is_breakdown_service
        : data.service_type === 'breakdown' ||
          data.service_type === 'repair' ||
          !!data.breakdown ||
          !!data.breakdown_id ||
          !!fallback.breakdownId;
    const statusVal = data.status
      ? data.status.charAt(0).toUpperCase() + data.status.slice(1).toLowerCase()
      : fallback.status || 'Completed';
    const cl =
      data.checklist || data.checklist_detail || fallback.checklist || {};
    ...
    const machineId =
      data.machine?.id || data.machine_id || fallback.machineId || null;
    const machineName =
      data.machine?.name ||
      data.machine?.equipment_name ||
      data.machine_name ||
      fallback.machineName ||
      (machineId ? `Machine #${machineId}` : '');
    const breakdownId =
      data.breakdown?.id || data.breakdown_id || fallback.breakdownId || null;
    const breakdownTicketNumber =
      data.breakdown?.ticket_number || fallback.breakdownTicketNumber || '';

    // Checklist extraction
    const oilDone =
      typeof cl.oil_change === 'object'
        ? cl.oil_change?.done
        : cl.oil_change === true ||
          cl.oil_change === 'Yes' ||
          fallback.oilChange === 'Yes';
    const oilAmt =
      typeof cl.oil_change === 'object'
        ? cl.oil_change?.amount || 0
        : cl.oil_change_amount !== undefined
          ? cl.oil_change_amount
          : fallback.oilChangeAmount || 0;

    const hydDone =
      typeof cl.hydraulic_oil === 'object'
        ? cl.hydraulic_oil?.done
        : cl.hydraulic_oil === true ||
          cl.hydraulic_oil === 'Yes' ||
          fallback.hydraulicOil === 'Yes';
    const hydAmt =
      typeof cl.hydraulic_oil === 'object'
        ? cl.hydraulic_oil?.amount || 0
        : cl.hydraulic_oil_amount !== undefined
          ? cl.hydraulic_oil_amount
          : fallback.hydraulicOilAmount || 0;

    const gearDone =
      typeof cl.gear_oil === 'object'
        ? cl.gear_oil?.done
        : cl.gear_oil === true ||
          cl.gear_oil === 'Yes' ||
          fallback.gearOil === 'Yes';
    const gearAmt =
      typeof cl.gear_oil === 'object'
        ? cl.gear_oil?.amount || 0
        : cl.gear_oil_amount !== undefined
          ? cl.gear_oil_amount
          : fallback.gearOilAmount || 0;

    const fuelDone =
      typeof cl.filters?.fuel_filter === 'object'
        ? cl.filters.fuel_filter?.done
        : cl.fuel_filter_change === true ||
          cl.fuel_filter_change === 'Yes' ||
          fallback.fuelFilterChange === 'Yes';
    const fuelAmt =
      typeof cl.filters?.fuel_filter === 'object'
        ? cl.filters.fuel_filter?.amount || 0
        : cl.fuel_filter_change_amount !== undefined
          ? cl.fuel_filter_change_amount
          : fallback.fuelFilterChangeAmount || 0;

    const oilFDone =
      typeof cl.filters?.oil_filter === 'object'
        ? cl.filters.oil_filter?.done
        : cl.oil_filter_change === true ||
          cl.oil_filter_change === 'Yes' ||
          fallback.oilFilterChange === 'Yes';
    const oilFAmt =
      typeof cl.filters?.oil_filter === 'object'
        ? cl.filters.oil_filter?.amount || 0
        : cl.oil_filter_change_amount !== undefined
          ? cl.oil_filter_change_amount
          : fallback.oilFilterChangeAmount || 0;

    const spList = data.spare_parts || fallback.spareParts || [];
    const sparePartsChange =
      (spList && spList.length > 0) || data.spare_parts_changed ? 'Yes' : 'No';

    const startTimeRaw =
      data.downtime?.start ||
      data.downtime_start ||
      fallback.serviceStartTime ||
      '';
    const endTimeRaw =
      data.downtime?.end || data.downtime_end || fallback.serviceEndTime || '';
    const downtimeMinutes =
      data.downtime?.minutes !== undefined
        ? data.downtime.minutes
        : data.downtime_minutes !== undefined
          ? data.downtime_minutes
          : fallback.downtimeMinutes || 0;

    const baseAmount =
      data.totals?.base_service_amount !== undefined
        ? data.totals.base_service_amount
        : data.base_service_amount !== undefined
          ? data.base_service_amount
          : fallback.serviceTypeAmount || 0;
    const totalCost =
      data.totals?.total_amount !== undefined
        ? data.totals.total_amount
        : data.total_amount !== undefined
          ? data.total_amount
          : data.total_cost || fallback.totalCost || 0;

    const sType = isBreakdown
      ? 'Repair'
      : data.service_type_label || fallback.serviceType || 'General Service';

    return {
      id: data.id || fallback.id,
      ticketNumber:
        data.ticket_number ||
        data.breakdown_ticket ||
        fallback.ticketNumber ||
        (data.id ? `#${data.id}` : ''),
      breakdownId: breakdownId,
      breakdownTicketNumber: breakdownTicketNumber,
      machineId: machineId,
      ...
      odometerReading:
        data.hours_odometer_reading !== undefined
          ? data.hours_odometer_reading
          : fallback.odometerReading || '',
      kmRun: data.km_run !== undefined ? data.km_run : fallback.kmRun || '',
      timeGap:
        data.time_gap_months !== undefined
          ? data.time_gap_months
          : fallback.timeGap || '',
      serviceType: sType,
      ...
      spareParts: (spList || []).map((p: any) => ({
        ...
        partName:
          p.part_name ||
          p.partName ||
          p.inventory_product?.name ||
          p.store_product?.product_name ||
          p.store_product?.name ||
          '',
  ...
  editService(service: any) {
    ...
    this.serviceRecordService
      .getServiceRecordById(service.id)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res: any) => {
        ...
        error: (err: any) =>
          console.error('Error fetching service record details for edit:', err),
      });
  }

  populateEditForm(parsed: any) {
    const machine = this.machines.find(
      (m) => m.id === parsed.machineId || m.name === parsed.machineName,
    );
    const finalMachineId = machine ? machine.id : parsed.machineId || null;

    if (parsed.breakdownId) {
      const existingBreakdownIndex = this.activeBreakdowns.findIndex((b: any) => b.id === parsed.breakdownId);
      const ticketNumber = parsed.breakdownTicketNumber || `BRK-${parsed.breakdownId}`;
      
      if (existingBreakdownIndex === -1) {
        this.activeBreakdowns = [...this.activeBreakdowns, {
          id: parsed.breakdownId,
          ticketNumber: ticketNumber,
          machineId: parsed.machineId,
          equipmentName: parsed.machineName || '',
          type: 'Breakdown',
          description: 'Loaded from service record'
        }];
      } else if (parsed.breakdownTicketNumber && this.activeBreakdowns[existingBreakdownIndex].ticketNumber !== parsed.breakdownTicketNumber) {
        const updatedBreakdowns = [...this.activeBreakdowns];
        updatedBreakdowns[existingBreakdownIndex] = {
          ...updatedBreakdowns[existingBreakdownIndex],
          ticketNumber: parsed.breakdownTicketNumber
        };
        this.activeBreakdowns = updatedBreakdowns;
      }
    }
    ...
    this.spareParts.clear();
    if (
      parsed.sparePartsChange === 'Yes' &&
      parsed.spareParts &&
      parsed.spareParts.length > 0
    ) {
      parsed.spareParts.forEach((p: any) => {
        this.spareParts.push(
          this.fb.group({
            ...
            storeId: [
              p.storeId || null,
              p.source === 'Store' ? Validators.required : null,
            ],
    ...
    this.serviceRecordService
      ...
      .subscribe({
        next: (res: any) => {
          const data = res?.data || res;
          if (data) {
            this.viewModalData = this.parseServiceRecordData(
              data,
              this.viewModalData,
            );
          }
        ...
        error: (err: any) =>
          console.error('Error fetching service record details for view:', err),
      });
  }
  ...
  submitBreakdown() {
    if (this.breakdownForm.valid) {
      ...
      const payload = {
        ...
        equipment_id:
          this.breakdownMachines.find((m) => m.machine_id === val.machineId)
            ?.category_id || null,
        ...
        .subscribe({
          next: (res: any) => {
            ...
            if (res && (res.status === 200 || res.status === 201)) {
              this.notificationService.show(
                res.message || 'Breakdown logged successfully!',
                'success',
                3000,
              );
              this.fetchActiveBreakdowns();
              ...
              this.notificationService.show(
                res.message || 'Failed to log breakdown',
                'error',
                3000,
              );
            }
          ...
          error: (err: any) => {
            ...
            this.notificationService.show(
              err.error?.message || err.message || 'Failed to log breakdown',
              'error',
              3000,
            );
          },
  ...
  submitService() {
    if (this.serviceForm.valid) {
      ...
      formData.append(
        'time_gap_months',
        val.timeGap ? val.timeGap.toString() : '',
      );
      formData.append(
        'base_service_amount',
        (val.serviceTypeAmount || 0).toString(),
      );

      // Checklist
      formData.append(
        'checklist[oil_change]',
        val.oilChange === 'Yes' ? 'true' : 'false',
      );
      formData.append(
        'checklist[oil_change_amount]',
        (val.oilChangeAmount || 0).toString(),
      );

      formData.append(
        'checklist[hydraulic_oil]',
        val.hydraulicOil === 'Yes' ? 'true' : 'false',
      );
      formData.append(
        'checklist[hydraulic_oil_amount]',
        (val.hydraulicOilAmount || 0).toString(),
      );

      formData.append(
        'checklist[gear_oil]',
        val.gearOil === 'Yes' ? 'true' : 'false',
      );
      formData.append(
        'checklist[gear_oil_amount]',
        (val.gearOilAmount || 0).toString(),
      );

      formData.append(
        'checklist[fuel_filter_change]',
        val.fuelFilterChange === 'Yes' ? 'true' : 'false',
      );
      formData.append(
        'checklist[fuel_filter_change_amount]',
        (val.fuelFilterChangeAmount || 0).toString(),
      );

      formData.append(
        'checklist[oil_filter_change]',
        val.oilFilterChange === 'Yes' ? 'true' : 'false',
      );
      formData.append(
        'oil_filter_change_amount',
        val.oilFilterChangeAmount?.toString() || '0',
      );
      formData.append(
        'spare_parts_changed',
        val.sparePartsChange === 'Yes' ? 'true' : 'false',
      );
      ...
      if (
        val.sparePartsChange === 'Yes' &&
        val.spareParts &&
        val.spareParts.length > 0
      ) {
        val.spareParts.forEach((part: any, i: number) => {
          formData.append(
            `spare_parts[${i}][source]`,
            part.source.toLowerCase(),
          );

          if (part.source === 'Inventory') {
            if (part.partId) {
              formData.append(
                `spare_parts[${i}][inventory_product_id]`,
            ...
            if (part.partId) {
              formData.append(
                `spare_parts[${i}][store_product_id]`,
                part.partId.toString(),
              );
            }
            if (part.storeId) {
          ...
          formData.append(
            `spare_parts[${i}][quantity]`,
            (part.quantity || 1).toString(),
          );
          formData.append(
            `spare_parts[${i}][amount]`,
            (part.amount || 0).toString(),
          );
        });
      ...
      if (val.serviceStartTime) {
        formData.append(
          'downtime_start',
          val.serviceStartTime.length === 5
            ? `${val.serviceStartTime}:00`
      ...
      if (val.serviceEndTime) {
        formData.append(
          'downtime_end',
          val.serviceEndTime.length === 5
            ? `${val.serviceEndTime}:00`
            : val.serviceEndTime,
        );
      }
      ...
      const apiCall = this.editingServiceId
        ? this.serviceRecordService.updateServiceRecord(
      ...
      apiCall.pipe(takeUntil(this.destroy$)).subscribe({
        next: (res: any) => {
          this.isSubmitting = false;
          this.notificationService.show(
            res?.message || 'Service record saved successfully',
            'success',
            3000,
          );
          this.closeModals();
        ...
        error: (err: any) => {
          ...
          this.notificationService.show(
            err.error?.message ||
              err.message ||
              'Error saving service record. Please check the network tab for details.',
            'error',
            3000,
          );
        },
